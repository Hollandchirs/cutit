...
import { analyzeTranscriptWithGemini } from './services/geminiService';
import { transcribeWithGroq } from './services/groqWhisperService';
...
export default function App() {
  ...
  // Ref to store state before drag for undo batching
  const segmentsBeforeDragRef = useRef<TimelineSegment[] | null>(null);
  ...
  // Start resize - save state for undo
  const handleResizeStart = useCallback(() => {
    segmentsBeforeDragRef.current = [...timelineSegments];
  }, [timelineSegments]);

  // Resize segment (trim start/end) - no history during drag
  const handleResizeSegment = useCallback((id: string, newStart: number, newEnd: number) => {
    setSegmentsNoHistory(prev => prev.map(seg =>
      seg.id === id
  ...
  // End resize - commit to history as single operation
  const handleResizeEnd = useCallback(() => {
    if (segmentsBeforeDragRef.current) {
      commitToHistory(segmentsBeforeDragRef.current);
      segmentsBeforeDragRef.current = null;
    }
  }, [commitToHistory]);
  ...
  const handleAnalyze = async () => {
    ...
    if (!groqApiKey || groqApiKey === 'your_groq_api_key_here') {
      alert('请在 .env 文件中配置 GROQ_API_KEY\n\n获取方式: https://console.groq.com/keys');
      return;
    }

    setStatus(ProcessingStatus.ANALYZING);
    setAnalysisProgress({ percent: 0, message: 'Preparing...' });

    const updateProgress = (message: string) => {
      let percent = 10;
      if (message.includes('Extracting')) percent = 20;
      if (message.includes('Decoding')) percent = 30;
      if (message.includes('Converting')) percent = 40;
      if (message.includes('Sending')) percent = 50;
      if (message.includes('Processing')) percent = 70;
      if (message.includes('Transcribed')) percent = 90;
      if (message.includes('Building')) percent = 95;
    ...
    try {
      ...
      // Process each clip with Groq Whisper
      for (let i = 0; i < clips.length; i++) {
        const clip = clips[i];
        updateProgress(`[${i + 1}/${clips.length}] Processing ${clip.name}...`);

        console.log(`\n📹 Transcribing: ${clip.name}`);

        // Use Groq Whisper API for fast, accurate transcription
        const whisperResult = await transcribeWithGroq(clip.file, groqApiKey, updateProgress);

        console.log(`[Groq] ${clip.name}: ${whisperResult.segments.length} segments transcribed`);

        // Step 2: Analyze with Gemini for intelligent duplicate detection
        updateProgress(`[${i + 1}/${clips.length}] Analyzing with Gemini AI...`);
        console.log(`[Gemini] Analyzing transcript for ${clip.name}...`);

        const geminiResult = await analyzeTranscriptWithGemini(whisperResult, clip.duration, updateProgress);

        console.log(`[Gemini] ${clip.name}: ${geminiResult.segments.length} segments analyzed`);

        // Build color map for groups
        const groupColors: Record<string, string> = {};

        // Convert Gemini segments to timeline segments
        geminiResult.segments.forEach((seg, idx) => {
          // Assign consistent color based on groupId
        ...
        setClips(prev => prev.map(c =>
          c.id === clip.id
            ? { ...c, status: 'done' as const, analysis: geminiResult }
            : c
        ));
      }

      updateProgress('Building timeline...');
    ...
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <MediaPool
          ...
          currentTime={timelineData ? timelineData.segment.range.start + timelineData.offset : 0}
          isPlaying={isPlaying && !previewClipId}
        />
